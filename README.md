...
The clean result is worth nothing today and the same file says why. There is no
source tree here yet, so the search covered nothing, and an empty tree passing a
search is not a property. What will be worth something is a run of the whole
pipeline with no network route available, and that does not exist either.
